...
def run():
    env = gym.make(ENV)
    a2c = A2C(env, SUMMARY_DIR, gpu=True)
    rew_list = []
    ...
    for episode in range(EPISODE_MAX):
        ...
        while True:
            ...
            a, v = a2c.choose_state(s, training=True)
            # Update ppo
            if t == STEPBATCH:  # or (terminal and t < BATCH):
                ...
                bs, ba, br, badv = np.reshape(rollout_s, (t,) + a2c.s_dim), np.vstack(rollout_a), \
                                   np.vstack(returns), np.vstack(advantage)

                graph_summary = a2c.train(bs, ba, br, badv)
                rollout_s, rollout_a, rollout_r, rollout_v, rollout_terminal = [], [], [], [], []
                t = 0
            rollout_s.append(s)
            rollout_a.append(a)
            ...
            if terminal:
                ...
                a2c.writer.add_summary(worker_summary, episode)
                try:
                    a2c.writer.add_summary(graph_summary, episode)
                except NameError:
                    pass
                a2c.writer.flush()
                ...
                if episode % 500 == 0 and episode > 0:
                    path = a2c.save_model(SUMMARY_DIR, episode)
